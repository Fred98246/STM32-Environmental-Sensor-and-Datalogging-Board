...
#define CS_HIGH()	{HAL_GPIO_WritePin(GPIOB, SPI_CS1_Pin, GPIO_PIN_SET);}
#define CS_LOW()	{HAL_GPIO_WritePin(GPIOB, SPI_CS1_Pin, GPIO_PIN_RESET);}
...
{
	BYTE rxDat;
	HAL_SPI_TransmitReceive(&SD_SPI_HANDLE, (uint8_t*)&dat, (uint8_t*)&rxDat, 1, 50);
    return rxDat;
}
...
{
    // Actually transmit the buffer to the SD card!
